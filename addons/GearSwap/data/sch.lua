...
---------------------------	
	function customize_idle_set(idleSet)
		...
		if player.mpp < 60 then 
			send_command('input //send Enuriel /ma "Mage\'s Ballad III Enurihel')
			send_command('@wait 5; input //send Enuriel /ma "Mage\'s Ballad II Enurihel')
			send_command('@wait 9; input //send Enuriel /ma "Mage\'s Ballad Enurihel')
		end 
...
-----------------------
	function job_precast(spell, action, spellMap, eventArgs) 
		if not buffactive['Addendum: White'] and (spell.skill =="Healing Magic" or spell.skill == "Enhancing Magic") then 
			cancel_spell()
			send_command('input /ja "Light Arts" <me>')
			send_command('@wait 1; input /ja "Addendum: White" <me>')		
		end
		if spell.english == 'Protect V' and not buffactive['Accession'] and not buffactive['Protect'] then 
			cancel_spell()
			send_command('@wait 1; input /ja "Accession" <me>')
			send_command('@wait 3; input /ma "Protect V" <me>')
		end 
		if spell.english == 'Shell V' and not buffactive['Accession'] and not buffactive['Shell'] then 
			cancel_spell()
			send_command('@wait 1; input /ja "Accession" <me>')
			send_command('@wait 3; input /ma "Shell V" <me>')
		end
	end 
